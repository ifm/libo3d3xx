  ...
  try
    {
      ...
      o3d3xx::CmdLineOpts opts("Off-board Cartesian Computation Example");
      po::options_description cartesian_opts("Cartesian Computation Options");
      cartesian_opts.add_options()
        ("nframes,n",
         po::value<int>()->default_value(1),
         "Number of frames to process");

      opts.visible.add(cartesian_opts);
      ...
      // Get access to the camera and an image buffer to hold and organize the
      ...
      // Initialize the framegrabber, with a schema that only streams in the
      // unit vectors and get the unit vectors. We only need to get them once.
      ...
      // Reinitialize the framegrabber with a schema that streams only the
      // radial distance image. NOTE: The extrinsics will be available as part
      // of this schema as they are an invariant.
      ...
      for (int i = 0; i < nframes; ++i)
        {
          ...
          // NOTE: The unit vectors are already rotated, so, we only need to
          // extract out the translation vector. The units are in mm.
          ...
          // Transform from the IFM/O3D camera frame to the libo3d3xx/ROS camera
          // frame
    ...
    {
      std::cerr << "Failed to stream cartesian data: "
                << ex.what() << std::endl;
      return 1;
    }
