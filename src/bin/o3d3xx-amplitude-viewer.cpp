...
public:
  CmdLineOpts(int argc, const char **argv)
    : visible("Visualize Amplitude Images from O3D3XX Camera")
  {
    // generic options
...
int main(int argc, const char **argv)
{
  ...
  try
      ...
      if (opts.vm.count("xmlrpc-port"))
	{
      ...
      cv::Mat colormap_img;
      cv::namedWindow("o3d3xx Amplitude Image", cv::WINDOW_NORMAL);
      double min, max;
	  ...
	  if (fg->WaitForFrame(buff.get(), 1000))
	    {
	      ...
	      cv::imshow("o3d3xx Amplitude Image", colormap_img);
	    }
